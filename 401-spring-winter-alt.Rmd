...
ggplot(data) + geom_point(aes(x=Position, y=log10p), color="grey50", alpha=0.75, cex=0.5) +
  ...
  geom_vline(xintercept = 26078281-1500000) +
  geom_vline(xintercept = 26078281+1500000) +
  theme_bw() +
...
```{sh, eval=FALSE}
srun -p med -t 16:00:00 --nodes=1 bcftools mpileup --min-BQ 20 --min-MQ 20 -Ou --fasta-ref ~/genomes/chinook/GCF_002872995.1_Otsh_v1.0_genomic.fna -r NC_037109.1:24578281-27578281 -b bamlists/bamlist156-winter-spring.bamlist | \
bcftools call -mv -Ov -o outputs/401/bamlist156-chrom13.vcf > outputs/401/chrom13.out 2> outputs/401/chrom13.err &

#bgzip and bcftools index files, then
srun -p med -t 8:00:00 --nodes=1 bcftools norm -c w outputs/401/bamlist156-chrom13.vcf.gz -f ~/genomes/chinook/GCF_002872995.1_Otsh_v1.0_genomic.fna -Ov -o outputs/401/bamlist156-chrom13-norm.vcf 
...
```{sh, eval=FALSE}
vcftools --gzvcf outputs/401/bamlist156-chrom13.vcf.gz --min-alleles 2 --max-alleles 2 --max-missing 0.4 --maf 0.05 --out outputs/401/bamlist156-chrom13-filtered --recode 

```
17214 out of 41713 sites kept.    

Moving locally for haplo raster      
Recoding names       
```{sh, eval=FALSE}
bcftools reheader --samples outputs/301/newnames -o outputs/401/bamlist156-chrom13-filtered.recode.renamed.vcf outputs/401/bamlist156-chrom13-filtered.recode.vcf
```

1. Imputing (all together now)
```{sh, eval=FALSE}
java -jar ~/bin/beagle.27Jan18.7e1.jar gl=outputs/401/bamlist156-chrom13-filtered.recode.renamed.vcf out=outputs/401/beagle-imputed > outputs/401/beagle-impute-stdout.txt 
```

2. Phasing    
```{sh, eval=FALSE}
java -jar ~/bin/beagle.27Jan18.7e1.jar gt=outputs/401/beagle-imputed.vcf.gz out=outputs/401/beagle-imp-phased ibd=true ibdcm=0.05 > outputs/401/beagle-phase-stdout.txt 
```

3. Copying code from Eric A.    

```{r, eval=FALSE}
ibd <- read_tsv("outputs/401/beagle-imp-phased.ibd.gz",
...
Save for later!
```{r, eval=FALSE}
save(big_haps2, file = "outputs/401/big_haps2.rda")
```

```{r}
load("outputs/401/big_haps2.rda")
```
...
ggplot(test)+geom_raster(aes(x=as.character(POS), y=haplo_name, fill=allele2))+
  ...
  labs(fill = "Allele")+
  ggtitle(paste("Ots13 Haplo-Raster from ", min(big_haps2$POS), " to ", max(big_haps2$POS), sep="" )) +
  theme(plot.title = element_text(hjust = 0.5))

ggsave("outputs/401/Ots13-haplo-raster.pdf")
```

#Separate and impute vcfs of run types
```{sh, eval=FALSE}
bcftools view -S outputs/400/winter-samples.txt outputs/401/bamlist156-chrom13-filtered.recode.renamed.vcf > outputs/401/winter.vcf
bcftools view -S outputs/400/spring-samples.txt outputs/401/bamlist156-chrom13-filtered.recode.renamed.vcf > outputs/401/spring.vcf

java -jar ~/bin/beagle.27Jan18.7e1.jar gl=outputs/401/winter.vcf out=outputs/401/winter-imputed 
java -jar ~/bin/beagle.27Jan18.7e1.jar gl=outputs/401/spring.vcf out=outputs/401/spring-imputed 

```

Calculating some freqs:
```{sh, eval=FALSE}
vcftools --gzvcf outputs/401/winter-imputed.vcf.gz  --freq --out outputs/401/winter-imputed
vcftools --gzvcf outputs/401/spring-imputed.vcf.gz  --freq --out outputs/401/spring-imputed
```
...
```{r}
freq1<-read_tsv("outputs/401/winter-imputed.frq", skip = 1, col_names = FALSE) %>% 
...
freq2<-read_tsv("outputs/401/spring-imputed.frq", skip = 1, col_names = FALSE) %>% 
...
write_csv(merge,"outputs/401/Ots13-allele-freqs.csv")
...
```{sh, eval=FALSE}
bcftools index outputs/401/spring-imputed.vcf.gz
bcftools index outputs/401/winter-imputed.vcf.gz
bcftools merge -Oz -o outputs/401/ots13-imputed-combined.vcf.gz outputs/401/spring-imputed.vcf.gz outputs/401/winter-imputed.vcf.gz
```